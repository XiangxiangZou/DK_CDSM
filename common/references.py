...
def build_cartesian_reference(
    args: argparse.Namespace,
    ik_solver: MujocoSiteIK,
) -> dict[str, np.ndarray]:
    trajectory = getattr(args, "trajectory", "circle")
    cartesian = generate_cartesian_reference(
        CartesianReferenceConfig(
            ...
            inner_radius_ratio=getattr(args, "inner_radius_ratio", 0.382),
        )
    )
...
def build_cartesian_circle_reference(
    ...
    return build_cartesian_reference(args, ik_solver)
