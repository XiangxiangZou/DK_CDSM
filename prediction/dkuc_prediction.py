"""DKUC prediction method with its own training and evaluation entrypoint."""
...
def make_network_class():
    ...
    class DKUCNetwork(nn.Module):
        def __init__(self, config: DKUCConfig, state_dim: int = STATE_DIM, control_dim: int = CONTROL_DIM):
            super().__init__()
...
def curriculum_horizon(epoch: int, config: DKUCConfig) -> int:
...
def compute_loss(model, x_sequence, u_sequence, horizon: int, config: DKUCConfig):
    import torch
...
def fit_dkuc(
    ...
    DKUCNetwork = make_network_class()
    model = DKUCNetwork(config, train_states.shape[-1], train_inputs.shape[-1]).to(device)
    ...
    for epoch in range(1, config.epochs + 1):
        ...
        if val_value < best_value:
            ...
            best_epoch = epoch
            torch.save(model.state_dict(), output / "best_dkuc.pt")

    save_json(output / "model_config.json", {"model": "DKUC", "config": asdict(config)})
    np.savetxt(
        output / "dkuc_training_history.csv",
...
class DKUCModel:
    ...
    def __init__(self, artifact_dir: str | Path, device: str = "cuda"):
        ...
        self.config = DKUCConfig(**load_json(self.artifact_dir / "model_config.json")["config"])
        ...
        self._torch = torch
        DKUCNetwork = make_network_class()
        self.model = DKUCNetwork(self.config, self.state_dim, self.control_dim).to(self.device)
        state_dict = torch.load(self.artifact_dir / "best_dkuc.pt", map_location=self.device, weights_only=True)
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Train and evaluate the DKUC Koopman prediction method.",
...
def main() -> None:
    ...
    paths = create_prediction_run_paths("dkuc", args.run_type, args.tag, args.out_dir or None)
    ...
    model = DKUCModel(output, str(device))
    ...
    print(f"[done] DKUC artifacts -> {output}")
