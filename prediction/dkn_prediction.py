"""DKN prediction method with its own training and evaluation entrypoint."""
...
def make_network_class():
    ...
    class DKNNetwork(nn.Module):
        def __init__(self, config: DKNConfig, state_dim: int = STATE_DIM, control_dim: int = CONTROL_DIM):
            super().__init__()
            ...
            self.bound_lift = bool(config.bound_lift)
            self.encoder = mlp((self.state_dim, *tuple(config.hidden), self.lift_dim), config.activation)
            self.control_net = mlp(
                (self.state_dim + self.control_dim, *tuple(config.control_hidden), self.control_dim_hat),
                config.activation,
            )
        ...
        def koopman_step(self, z, u_norm):
            x_for_control = self.state_from_latent(z)
            v = self.control_net(torch.cat([x_for_control, u_norm], dim=-1))
            return self.A(z) + self.B(v)

    return DKNNetwork


def curriculum_horizon(epoch: int, config: DKNConfig) -> int:
...
def compute_loss(model, x_sequence, u_sequence, horizon: int, config: DKNConfig):
    import torch
...
def fit_dkn(
    ...
    DKNNetwork = make_network_class()
    model = DKNNetwork(config, train_states.shape[-1], train_inputs.shape[-1]).to(device)
    ...
    for epoch in range(1, config.epochs + 1):
        ...
        if val_value < best_value:
            ...
            best_epoch = epoch
            torch.save({"model_state": model.state_dict(), "config": asdict(config)}, output / "best_dkn.pt")

    save_json(output / "model_config.json", {"model": "DKN", "config": asdict(config)})
    np.savetxt(
        output / "dkn_training_history.csv",
...
class DKNModel:
    ...
    def __init__(self, artifact_dir: str | Path, device: str = "cuda"):
        ...
        self.config = DKNConfig(**load_json(self.artifact_dir / "model_config.json")["config"])
        ...
        self._torch = torch
        DKNNetwork = make_network_class()
        self.model = DKNNetwork(self.config, self.state_dim, self.control_dim).to(self.device)
        checkpoint = torch.load(self.artifact_dir / "best_dkn.pt", map_location=self.device, weights_only=True)
    ...
    def step_latent(self, z: np.ndarray, u_phys: np.ndarray, x_phys: np.ndarray | None = None) -> np.ndarray:
        del x_phys
        u_norm = self.u_normer.transform(np.asarray(u_phys).reshape(1, -1))[0].astype(np.float32)
        z_tensor = self._torch.from_numpy(np.asarray(z, dtype=np.float32).reshape(1, -1)).to(self.device)
        u_tensor = self._torch.from_numpy(u_norm.reshape(1, -1)).to(self.device)
        with self._torch.no_grad():
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Train and evaluate the DKN Koopman prediction method.",
...
def main() -> None:
    ...
    paths = create_prediction_run_paths("dkn", args.run_type, args.tag, args.out_dir or None)
    ...
    model = DKNModel(output, str(device))
    ...
    print(f"[done] DKN artifacts -> {output}")
