...
--------------------------------------------------------------------------
    outputs/simulation_logs/<program_name>/<basename>/
    ...
    (可选) outputs/figures/<program_name>/<utils_plot_timestamp>/<basename2>.gif
...
--------------------------------------------------------------------------
    ...
    方法 2 (动态: 跟着当时的轨迹回放):
        python utils_mujoco_log.py outputs/simulation_logs/<program_name>/<basename>/
...
--------------------------------------------------------------------------
    ...
    logger = MjSimLogger(
        ...
        extra_gif_save_dir=get_save_dir(),  # utils_plot 输出目录; None 则只保存一份
        extra_gif_basename="myrun_sim_playback",  # 额外副本文件名 (不含 .gif)
    )
    while ...:
...
def _init_log_dir(custom_name: Optional[str] = None):
    ...
    # 目录: outputs/simulation_logs/<program>/<program>_<ts>/
...
class MjSimLogger:
    ...
    extra_gif_save_dir : str | None         额外拷贝 GIF 的目录
