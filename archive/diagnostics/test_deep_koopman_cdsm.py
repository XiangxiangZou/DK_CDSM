...
本脚本会做的事
────────────────────────────────────────────────────────────────────────────
(1) 自动找到 outputs/figures/historical_runs/deep_koopman_cdsm/ 下最新一个时间戳目录, 加载:
...
(4) 4 张诊断图 + 1 份数值统计文本, 全部存到
        outputs/figures/test_deep_koopman_cdsm/<ts>/
...
────────────────────────────────────────────────────────────────────────────
    ...
    # 指定模型目录:
    python test_deep_koopman_cdsm.py --model_dir outputs/figures/historical_runs/deep_koopman_cdsm/20260512_152030
...
def find_latest_model_dir(
    root: str = "outputs/figures/historical_runs/deep_koopman_cdsm",
) -> Path:
    """在历史输出目录下找最新一次训练 (时间戳最大的子目录).
