...
from visualization.path_setup import DEFAULT_XML, PROJECT_ROOT
...
def load_log(result_dir: str | Path, model_name: str) -> Dict[str, np.ndarray]:
    ...
    path = root / f"closed_loop_{model_name}.npz"
    if not path.exists():
        path = root / "arrays" / f"closed_loop_{model_name}.npz"
    if not path.exists():
...
def main() -> None:
    ...
    save_json(out_dir / f"{stamp}_{args.trajectory}_mujoco_motion_metadata{suffix}.json", meta)
