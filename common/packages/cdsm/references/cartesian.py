...
4. `star`: 五角星轮廓轨迹，适合检查尖角和方向突变处的闭环响应。
...
TRAJECTORY_KINDS = ("figure8", "circle", "square", "star")
...
class CartesianReferenceConfig:
    ...
    参数:
        kind: 轨迹类型。当前支持 `figure8`、`circle`、`square` 和 `star`。
        ...
        time_scaling: 时间规划方式。`quintic` 使用五次多项式平滑插值；`linear` 保留
            原始匀速相位/线性分段，用于对照实验。
        inner_radius_ratio: `star` 轨迹的内半径/外半径比例。
    """
    ...
    inner_radius_ratio: float = 0.382
...
def _star_reference(
    *,
    ...
    """Generate a closed five-point star outline trajectory."""
    phase = (theta / (2.0 * np.pi)) % 1.0
    edge = np.floor(phase * 10.0).astype(np.int32)
    local = phase * 10.0 - edge
    edge = np.minimum(edge, 9)
    if time_scaling == "quintic":
        local = _quintic_smoothstep(local)

    ratio = float(inner_radius_ratio)
    if ratio <= 0.0 or ratio >= 1.0:
        raise ValueError("inner_radius_ratio must be in (0, 1)")
    angles = np.pi / 2.0 + np.arange(10, dtype=np.float64) * np.pi / 5.0
    scales = np.where(np.arange(10) % 2 == 0, 1.0, ratio)
    vertices = np.column_stack(
        [
            center_x + radius_x * scales * np.cos(angles),
            center_y + radius_y * scales * np.sin(angles),
        ]
    )
    next_vertices = np.roll(vertices, -1, axis=0)
    start = vertices[edge]
    end = next_vertices[edge]
    return (start + (end - start) * local[:, None]).astype(np.float64)
...
def generate_cartesian_reference(cfg: CartesianReferenceConfig) -> Dict[str, np.ndarray | dict | str]:
    ...
    if kind == "figure8":
        ...
        theta = float(cfg.phase) + 2.0 * np.pi * tau / float(cfg.period)
        xy_ref = _star_reference(
            theta=theta,
            center_x=float(cfg.center_x),
            center_y=float(cfg.center_y),
            radius_x=float(cfg.radius_x),
            radius_y=float(cfg.radius_y),
            inner_radius_ratio=float(cfg.inner_radius_ratio),
            time_scaling=time_scaling,
        )
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument("--inner_radius_ratio", type=float, default=0.382, help="star 轨迹内外半径比例。")
    ...
    parser.add_argument(
        "--out_dir",
        default=str(Path(__file__).resolve().parent / "results" / "cartesian_reference"),
        help="参考轨迹 npz 输出根目录。",
    )
